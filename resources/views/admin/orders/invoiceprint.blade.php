...
<body onload="window.print();">
    <table style="font-size: 13px; width: 720px; margin:0 auto; background:#fff;border-spacing: 0;font-family: 'Arial', sans-serif;">
        ...
        <tbody>
            ...
            <tr>
                <td><h2 style="text-align:center;font-weight: 500;text-transform: uppercase;">Tax Invoice</h2></td>
            </tr>
            <tr>
                <td>
                ...
                <td style="padding: 12px">
                    <strong>
                        ...
                        <?php if( $cond ) : ?>
                           ...
                           <?=$symbol?> <?=number_format($item['item_sale_price'] * $item['product_quantity'] * $order['currency_value'],2)?> <del><?=$symbol?> <?=number_format($item['item_price'] * $item['product_quantity'] * $order['currency_value'],2)?></del>
   ...
   <div style="text-align: right;font-size: 13px;width: 720px;margin:0 auto;font-family: 'Arial', sans-serif;background: #F6F6F6;-webkit-print-color-adjust: exact;border-radius: 20px;">
    <ul style="list-style-type: none; padding:12px">
        ...
        if( $productsdiscount != 0 ) : ?>
            ...
            <li style="padding:5px 0"><strong>Items total:</strong> <span style="margin-left: 10px;"><strong><?=$symbol?> <?= number_format($itemtotal , 2) ?></strong></span></li>
            <li style="padding:5px 0"><strong>Products Discount:</strong> <span style="margin-left: 10px;"><strong><?=$symbol?> <?= number_format($productsdiscount , 2) ?></strong></span></li>

        <?php endif;?>

        <li style="padding:5px 0"><strong>Subtotal:</strong> <span style="margin-left: 10px;"><strong><?=$symbol?> <?=number_format($subtotal,2)?></strong></span></li>

        <?php

        if($order['coupon_amount'] != 0 ) : $total = $total - $order['coupon_amount']; ?>
            <li style="padding:5px 0"><strong>Coupon (<?=$order['coupon_code']?>):</strong>
                <span style="margin-left: 10px;"><strong>-<?=$symbol?> <?=number_format($order['coupon_amount'])?></strong></span>
            </li>
        ...
        if( $order['credit_amount'] != 0 && $order['credit_amount'] != '' && $order['credit_amount'] != null ) : $total = $total - $order['coupon_amount']; ?>
            <li style="padding:5px 0"><strong>Credit:</strong>
                <span style="margin-left: 10px;"><strong>-<?=$symbol?> <?= number_format($order['credit_amount'] * $order['currency_value'],2) ?></strong></span>
            </li>

        <?php endif; ?>

        <li style="padding:5px 0"><strong>Shipping:</strong> 
            <span style="margin-left: 10px;"><strong><?=$order['shipping_cost'] == 0 ? 'Free Shipping' : $symbol.' '. number_format($order['shipping_cost'],2)?></strong></span>
        </li>

        <li style="padding:5px 0"><strong>VAT:</strong> 
            <span style="margin-left: 10px;"><strong><?=$symbol?> <?=number_format($order['vat'],2)?></strong></span>
        </li>

        <li style="padding:5px 0"><strong>Order Total:</strong> <span style="margin-left: 10px;"><strong><?=$symbol?>  <?= number_format($total , 2) ?>
    </strong></span></li>
